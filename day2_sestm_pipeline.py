...
def main():
    ...
    if (not args.force_retrain and vec_pkl.exists() and lda_pkl.exists() and clf_pkl.exists()):
        ...
        if not hasattr(vectorizer, "transform"):
            ...
            net_tone = lda.transform(vectorizer.transform(df["text"]))[:, 0] - lda.transform(vectorizer.transform(df["text"]))[:, 1]
            auc = roc_auc_score(df["label"], clf.predict_proba(lda.transform(vectorizer.transform(df["text"])))[:, 1])
